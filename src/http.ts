  ...
  return createServer(async(req,res)=>{
    try{
      ...
      if(url.pathname==='/'){
        ...
        res.end(`<!doctype html><html><head><meta charset="utf-8"><title>Threads Outreach Agent</title><style>body{font-family:system-ui;max-width:900px;margin:48px auto;padding:0 20px}code{background:#eee;padding:2px 6px;border-radius:5px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.card{border:1px solid #ddd;border-radius:12px;padding:18px}.n{font-size:32px;font-weight:700}</style></head><body><h1>Threads Outreach Agent</h1><p>Mode: <code>${agent.config.mode}</code> · Threads: <code>@${agent.ownUsername||'starting'}</code></p><div class="grid"><div class="card"><div class="n">${counts.leads}</div>Leads</div><div class="card"><div class="n">${counts.hot}</div>HOT</div><div class="card"><div class="n">${counts.drafts}</div>Drafts</div><div class="card"><div class="n">${counts.sent}</div>Sent</div></div><p>Hunter: ${agent.lastHunterAt??'—'}<br>Inbound: ${agent.lastInboundAt??'—'}<br>Content: ${agent.lastContentAt??'—'}<br>Public discovery: ${agent.publicDiscoveryHealthy===true?'OK':agent.publicDiscoveryHealthy===false?'BLOCKED':'UNKNOWN'}<br>Discovery issue: ${agent.discoveryIssue??'—'}<br>Last error: ${agent.lastError??'—'}</p><p>Private API: <code>/api/leads</code>, <code>/api/outreach</code>, <code>/api/run-hunter</code>, <code>/api/run-content</code>.</p></body></html>`);return;
      }
      if(url.pathname.startsWith('/api/')&&!authorized(req,agent.config.dashboardToken)){json(res,401,{error:'unauthorized'});return;}
      if(url.pathname==='/api/status'){json(res,200,{mode:agent.config.mode,username:agent.ownUsername,lastHunterAt:agent.lastHunterAt,lastInboundAt:agent.lastInboundAt,lastContentAt:agent.lastContentAt,lastError:agent.lastError,publicDiscoveryHealthy:agent.publicDiscoveryHealthy,discoveryIssue:agent.discoveryIssue,counts:await agent.db.counts()});return;}
