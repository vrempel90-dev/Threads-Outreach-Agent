...
if(!config.databaseUrl){
  ...
  const socialCrawl=new SocialCrawlClient(config.socialCrawl.apiKey,config.socialCrawl.baseUrl);
  const notifier=new OwnerNotifier(config.telegram.token,config.telegram.chatId,config.telegram.bindCode,db);
  const agent=new OutreachAgent(config,db,threads,llm,socialCrawl,notifier);
