...
const COMMERCIAL = /\b(нужен|нужна|нужно|ищу|ищем|кто\s+(?:сделает|разработает)|заказать|разработчик|стоимост|сколько\s+стоит|цена|внедрить|подключить|керек|іздеймін|бағасы)\b/iu;
const PAIN = /(не\s+успева|долго\s+отвеч|теря(?:ем|ются?)\s+(?:заяв|лид)|вручную|ручн(?:ая|ой)|рутин|много\s+одинаков|менеджер.{0,30}(?:не\s+успева|перегруж)|клиент.{0,30}(?:жд|уход)|өтінім.{0,20}жоғал|қолмен|үлгерм)/iu;
